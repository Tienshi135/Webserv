...
Client::Client(const Client &copy) : _client_fd(copy._client_fd), _read_buffer(copy._read_buffer), _bytes_expected(copy._bytes_expected), _bytes_read(copy._bytes_read)
{
...
std::string	Client::concatBuffer() const
{
	return std::string(this->_read_buffer.begin(), this->_read_buffer.end());
}